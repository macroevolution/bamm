...
Here, \code{models} is a vector of the models we wish to consider. In this example, we send the function the vector \code{0:3}, which corresponds to models $M_0$, $M_1$, $M_2$, and $M_3$. The \code{results} variable is a pairwise matrix of Bayes factors for the specified models. By default, they are always computed so that the model with the higher rank is the numerator, e.g., $M_2 / M_1$, not $M_1 / M_2$.
