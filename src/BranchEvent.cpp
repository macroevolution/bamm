...
BranchEvent::BranchEvent(double speciation, double lamshift, double extinction, double mushift, Node *x, Tree *tp, MbRandom* rp, double map, double scale){
...
void BranchEvent::incrementMapPosition(double ink){
	...
	if (temp > mapend){
		...
		if (getEventNode()->getAnc() == treePtr->getRoot()){
			...
			if (treePtr->getRoot()->getLfDesc() == getEventNode() & treePtr->getRoot()->getRtDesc()->getCanHoldEvent()){
				setEventNode(treePtr->getRoot()->getRtDesc());
				
			}else if (treePtr->getRoot()->getRtDesc() == getEventNode() & treePtr->getRoot()->getLfDesc()->getCanHoldEvent()){
