...
void TraitBranchHistory::printBranchHistory(void){
	...
	if (nEvents > 0){
		for (std::set<TraitBranchEvent*, comp_history>::iterator i = eventsOnBranch.begin(); i != eventsOnBranch.end(); i++){
...
TraitBranchEvent*  TraitBranchHistory::getLastEvent(TraitBranchEvent* x){
	...
	//int counter = 0;
	for (std::set<TraitBranchEvent*>::iterator myIt = eventsOnBranch.begin(); myIt != eventsOnBranch.end(); myIt++){
		//counter++;
